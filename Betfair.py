...
class HorseBothListItem:

    horse_name = ""
    selection_id = 0

    fair_bet = 0
    fair_bet_max_price = 0
    fair_lay = 0
    fair_lay_max_price = 0
...
"""
        ...
        #print(111111, market_catalogue_response)
        """
print(market_catalogue_response)
"""
        market_catalouge_loads = json.loads(market_catalogue_response)
        #print(2222222, market_catalouge_loads)
        #print(market_catalouge_loads['runners'])
        try:
            market_catalouge_results = market_catalouge_loads['result']
            #print(3333333, market_catalouge_results['runners'][0]['runnerName'])
...
def getSelectionId(marketCatalogueResult):
    if(marketCatalogueResult is not None):
        for market in marketCatalogueResult:
            ...
            #print(market['runners'][0]['runnerName'])
            return market['runners'][0]['selectionId']

def getRunnerName(marketCatalogueResult):
    if(marketCatalogueResult is not None):
        for market in marketCatalogueResult:
            return market['runners'][0]['runnerName']
...
"""
    ...
    #print(44444, market_book_response)    ################ Missing runner name here
    """
...
def printPriceInfoBook(market_book_result):
    if(market_book_result is not None):
        ...
        for marketBook in market_book_result:
            ...
            print(11111111, marketBook)
            for runner in runners:
                print ('Selection id is ' + str(runner['selectionId']))
                #print ('Runner name is' + str(runner['runerName']))
...
def printPriceInfoCatalogue(market_book_result):
    if(market_book_result is not None):
        print ('Please find Best three available prices for the runners')
        for marketBook in market_book_result:
            runners = marketBook['runners']
            print(2222222, marketBook)

            list = []

            for runner in runners:
                print ('Selection id is ' + str(runner['selectionId']) + "   " + 'RunnerName is ' + str(runner['runnerName']))

                current = HorseBothListItem()
                current.horse_name = str(runner['runnerName'])
                current.selection_id = str(runner['selectionId'])
...
if ( args < 3):
    ...
    sessionToken = "LBHNyXZQhSfoaqjyrnbZNKHIrmFf7FLTSt2cGc+/+Yw="
...
#runnerId = getRunnerName(marketCatalogueResult)
"""
print(marketid)
print(runnerId)
"""
market_book_result = getMarketBookBestOffers(marketid)
printPriceInfoCatalogue(marketCatalogueResult)
printPriceInfoBook(market_book_result)
#printPriceInfoCatalogue(marketCatalogueResult)
